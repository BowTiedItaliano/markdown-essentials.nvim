...
local function transform_text(mode, transformation)
	...
	if mode == "normal" then
		...
		-- Replace the line (using current line number instead of ".")
	...
	elseif mode == "visual" then
		...
		if start_line == end_line then
			...
			-- Fix: Ensure we get the complete selected text
			local selected_text = string.sub(line, start_col, end_col)
			local transformed_text = transformation(selected_text)

			-- Replace the selected text using the proper method
			vim.api.nvim_buf_set_text(
				0, -- current buffer
				start_line - 1, -- 0-indexed start row
				start_col - 1, -- 0-indexed start column
				start_line - 1, -- 0-indexed end row
				end_col, -- 0-indexed end column
				{ transformed_text } -- replacement text as table of lines
			)
		else
			-- Handle multi-line visual selection
			for i = start_line, end_line do
				local line = vim.fn.getline(i)
				-- For first line, only apply from start_col to end of line
				if i == start_line then
					local front_part = string.sub(line, 1, start_col - 1)
					local selected_part = string.sub(line, start_col)
					local transformed_part = transformation(selected_part)
					vim.fn.setline(i, front_part .. transformed_part)
				-- For last line, only apply from start to end_col
				elseif i == end_line then
					local selected_part = string.sub(line, 1, end_col)
					local end_part = string.sub(line, end_col + 1)
					local transformed_part = transformation(selected_part)
					vim.fn.setline(i, transformed_part .. end_part)
				-- For middle lines, transform the whole line
				else
...
function M.toggle_bold(mode)
	transform_text(mode, function(text)
		if string.match(text, "^%*%*.*%*%*$") then
			-- Remove bold (only if the entire text is bold)
...
function M.toggle_italic(mode)
	transform_text(mode, function(text)
		if string.match(text, "^%*.*%*$") and not string.match(text, "^%*%*.*%*%*$") then
			-- Remove italic (only if the entire text is italic)
			return string.gsub(text, "^%*(.-)%*$", "%1")
		else
			-- Add italic
			return "*" .. text .. "*"
...
function M.setup(opts)
	...
	vim.keymap.set("n", keymaps.toggle_bold, function()
		M.toggle_bold("normal")
	end, { desc = "Toggle bold" })
	vim.keymap.set("n", keymaps.toggle_italic, function()
		M.toggle_italic("normal")
	end, { desc = "Toggle italic" })
	vim.keymap.set("n", keymaps.toggle_todo, function()
		M.toggle_todo("normal")
	end, { desc = "Toggle todo" })
	vim.keymap.set("n", keymaps.toggle_todo_done, function()
		M.toggle_todo_done("normal")
	end, { desc = "Toggle todo done" })
	vim.keymap.set("n", keymaps.toggle_bullet, function()
		M.toggle_bullet("normal")
	end, { desc = "Toggle bullet list" })
	vim.keymap.set("n", keymaps.toggle_numbered, function()
		M.toggle_numbered("normal")
	end, { desc = "Toggle numbered list" })
	vim.keymap.set("n", keymaps.make_header1, function()
		M.make_header("normal", 1)
	end, { desc = "Make header 1" })
	vim.keymap.set("n", keymaps.make_header2, function()
		M.make_header("normal", 2)
	end, { desc = "Make header 2" })
	vim.keymap.set("n", keymaps.make_header3, function()
		M.make_header("normal", 3)
	end, { desc = "Make header 3" })
	vim.keymap.set("n", keymaps.make_normal, function()
		M.make_normal("normal")
	end, { desc = "Make normal text" })

	-- Register visual mode keymaps
	vim.keymap.set("v", keymaps.toggle_bold, function()
		M.toggle_bold("visual")
	end, { desc = "Toggle bold" })
	vim.keymap.set("v", keymaps.toggle_italic, function()
		M.toggle_italic("visual")
	end, { desc = "Toggle italic" })
	vim.keymap.set("v", keymaps.toggle_todo, function()
		M.toggle_todo("visual")
	end, { desc = "Toggle todo" })
	vim.keymap.set("v", keymaps.toggle_todo_done, function()
		M.toggle_todo_done("visual")
	end, { desc = "Toggle todo done" })
	vim.keymap.set("v", keymaps.toggle_bullet, function()
		M.toggle_bullet("visual")
	end, { desc = "Toggle bullet list" })
	vim.keymap.set("v", keymaps.toggle_numbered, function()
		M.toggle_numbered("visual")
	end, { desc = "Toggle numbered list" })
	vim.keymap.set("v", keymaps.make_header1, function()
		M.make_header("visual", 1)
	end, { desc = "Make header 1" })
	vim.keymap.set("v", keymaps.make_header2, function()
		M.make_header("visual", 2)
	end, { desc = "Make header 2" })
	vim.keymap.set("v", keymaps.make_header3, function()
		M.make_header("visual", 3)
	end, { desc = "Make header 3" })
	vim.keymap.set("v", keymaps.make_normal, function()
		M.make_normal("visual")
	end, { desc = "Make normal text" })
